...
import com.example.emos.api.controller.form.reimform.DeleteReimByIdForm;
import com.example.emos.api.controller.form.reimform.InsertReimForm;
import com.example.emos.api.controller.form.reimform.SearchReimByIdForm;
import com.example.emos.api.controller.form.reimform.SearchReimByPageForm;
...
public class ReimController {
    ...
    public R insert(@Valid @RequestBody InsertReimForm form) {
        // 格式验证content 属性是否是 json 数组
        if (!JSONUtil.isJsonArray(form.getContent())) {
            return R.error("content不是JSON数组格式");
        }
