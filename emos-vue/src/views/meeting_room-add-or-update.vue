		...
		return {
			...
			dataRule: {
				name: [
					...
					{ required: true, pattern: '^[a-zA-Z0-9\u4e00-\u9fa5]{2,20}$', message: '会议室名称格式错误' },
				],
				max: [
					{ required: true,  message: '会议室人数上限不能为空' },
					{ required: true, pattern: '^[1-9]\\d{0,4}$', message: '数字格式错误' },
				]
			}
		};
	},
	...
	methods: {
		...
		dataFormSubmit: function() {
			...
			that.$refs['dataForm'].validate(valid => {
				if(valid) {
					that.$http(`meeting_room/${that.dataForm.id ? 'update' : 'insert'}`, 'POST', that.dataForm, true, function(resp) {
						if(resp.rows == 1) {
							that.$message({
								message: '操作成功',
								type: 'success',
