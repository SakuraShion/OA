<template>
	<el-dialog title="设置头像" width="500px" :close-on-click-modal="false" v-model="visible" :show-close="false">
		<el-upload
			ref="upload"
			:action="url"
			:limit='1'
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" @click="setPhoto()" size="medium" :disabled="disableBtn">{{ btn }}</el-button>
			</span>
		</template>
	</el-dialog>
</template>
...
export default {
	data: function() {
		return {
			visible: false,
			url: this.$baseUrl + 'cos/uploadCosFile?type=photo',
			photoUrl: null,
			picList: {},
			disableBtn: false,
			btn: '确定'
	...
	methods: {
		...
		successHandle: function(resp, file, fileList) {
			if(resp && resp.code == 200) {
				for(let one of fileList) {
					...
					this.photoUrl = one.response.url;
				}
			} else {
		...
		setPhoto:function(){
			...
			that.$http("user/setPhoto","POST",data,true,function(resp){
				that.$message({
					message: '操作成功',
					type: 'success',
					duration: 1200,
