...
import com.example.emos.api.task.MailTask;
...
public class UserServiceImpl implements UserService {
    ...
    public int insert(TbUser user) {
        if ("男".equals((String) user.getSex())) {
            user.setPhoto("http://120.46.140.24/emos/defaultMan.png");
        } else if ("女".equals((String) user.getSex())) {
            user.setPhoto("http://120.46.140.24/emos/defaultWoman.png");
        }

        int rows = userDao.insert(user);
        if (rows == 1) {
            mailTask.sendMail(user.getId());
        }
        return rows;
    }
    ...
    @Override
    public int setPhoto(HashMap param) {
        return userDao.setPhoto(param);
    }
